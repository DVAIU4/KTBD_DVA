...
if (isset($_POST['add_defect'])) {
    $sql = "INSERT INTO defects (dfct_name, dfct_type, dfct_desc, dfct_date) 
...
if (isset($_GET['delete'])) {
    $sql = "DELETE FROM defects WHERE dfct_id = :id";
...
<head>
    ...
    <style type="text/css">
        ...
        .form-container textarea { /* Add textarea here */
            width: 95%;
            padding: 10px;
        ...
        .form-container textarea:focus { /* Add textarea here */
            border-color: #000;
            outline: none;
...
<div class="content-area">
    <table class="data-table">
        ...
        <?php foreach ($defects as $defect): ?>
            <tr>
                <td><?= htmlspecialchars($defect['DFCT_ID']); ?></td>
                <td><?= htmlspecialchars($defect['DFCT_NAME']); ?></td>
                <td><?= htmlspecialchars($defect['DFCT_TYPE']); ?></td>
                <td><?= htmlspecialchars($defect['DFCT_DESC']); ?></td>
                <td><?= htmlspecialchars($defect['DFCT_DATE']); ?></td>
                <td>
                    <a href="?delete=<?= $defect['DFCT_ID'] ?>"
    ...
    <div class="form-container">
        ...
        <form method="post">
            ...
            <div class="form-group">
                <label for="dfct_type">Тип:</label>
                <select id="dfct_type" name="dfct_type" required>
                    <option value="else">не выбрано</option>
                    <option value="connection">connection</option>
                    <option value="wrongval">wrongval</option>
                    <option value="otherdef">otherdef</option>
                </select>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label for="d_date">Дата (ДД.ММ.ГГ):</label>
                <input type="text" id="d_date" name="d_date" required
                       placeholder="ДД.ММ.ГГ"
                       pattern="\d{2}\.\d{2}\.\d{2}"
                       title="Введите дату в формате ДД.ММ.ГГ">
            </div>
            <button type="submit" name="add_defect" class="form-submit-button">Добавить</button>
        </form>
    </div>
</div>
